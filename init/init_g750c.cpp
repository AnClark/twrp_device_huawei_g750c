...
/**
 * Define android::init::vendor_load_properties().
 * 
 * Omni 8.1 has a bug in the definition of entrance function android::init::vendor_load_properties(). 
 * Here it is:
 ...
 * Macro TARGET_INIT_VENDOR_LIB is defined via "-D" by init's Android.mk when
 * TARGET_INIT_VENDOR_LIB is set in BoardConfig.mk. However, in Oreo, it
 * doesn't work.
 * 
 * So I have to manually add a namespace definition in libinit_g750c.cpp. This 
 * may cause warning or error on Android Pie, but works well with Omni 8.1.
 */
namespace android {
	namespace init {
...
void android::init::vendor_load_properties()
{
